...
async def startup():
  global client
  # bump timeout; devnet can be slow
...
async def root():
  return {
    "ok": True,
    "service": "solapi",
    "endpoints": ["/init-user", "/post", "/like", "/read-post/{sig}", "/read-user/{owner}"],
  }
...
async def init_user(req: InitUserReq):
  ...
  # Wait briefly until the PDA is readable (not strictly required)
  ...
  # Return 202 if still not visible
...
async def post_text(req: PostReq):
  ...
  for i, part in enumerate(parts, start=1):
    ixs.append(
      pack_post_ix(
        owner,
        is_head=(i == 1),        # first chunk is HEAD
        chunk_id=i,
...
async def read_post(sig: str):
  # Handle RPC slowness/timeouts as 404 (retry on client)
  try:
